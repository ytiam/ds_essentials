...
class FeatureSelector():
    ...
    def identify_single_unique(self):
        ...
        record_single_unique = pd.DataFrame(unique_counts[unique_counts < 2]).reset_index().rename(columns = {'index': 'feature', 
                                                                                                                0: 'nunique'})
    ...
    def identify_zero_importance(self, task, eval_metric=None, 
        ...
        #    col_type = self.data.loc[:,c].dtype
        #    if col_type == 'object' or col_type.name == 'category':
        #        features.loc[:,c] =  pd.Categorical(self.data.loc[:,c])
        ...
        def self_split_train_test(features, labels,test_size=0.1,random_state=40):
            for i in range(6):
                unique_split_feature = sorted(features.index.unique())
                train_CLM,test_CLM=train_test_split(unique_split_feature,test_size=test_size,random_state=random_state+i*4)
                train_features = features.loc[features.index.isin(train_CLM),:]
                valid_features = features.loc[features.index.isin(test_CLM),:]
                train_labels = labels.loc[labels.index.isin(train_CLM)].values
                valid_labels = labels.loc[labels.index.isin(test_CLM)].values
                if abs(sum(valid_labels)/len(valid_labels)/sum(train_labels)*len(train_labels)-1)<0.1:
                    print('seed is',random_state+i*4)
        ...
        for _ in range(n_iterations):
            ...
            if early_stopping:                
                train_features, valid_features, train_labels, valid_labels = \
                    self_split_train_test(features, self.labels,test_size=0.1,random_state=40+_*10)
            
                # Train the model with early stopping
                model.fit(train_features, train_labels, eval_metric = eval_metric,
                          eval_set = [(valid_features, valid_labels)],
                          early_stopping_rounds = 100, verbose = -1)
                pred=model.predict_proba(valid_features)[:,1]
                
                average_precision = average_precision_score(valid_labels, pred)
    ...
    def identify_low_importance(self, cumulative_importance):
        ...
        print('%d features do not contribute to cumulative importance of %0.3f.\n' % (len(self.ops['low_importance']),
    ...
    def identify_all(self, selection_params):
        ...
        print('%d total features out of %d identified for removal totally.\n' % (self.n_identified, 
